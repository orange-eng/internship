                ...
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in det:    #烟头的坐标保存在xyxy里面
                        ...
                        if save_img or view_img:  # Add bbox to image
                            ...
                            #     if conf > 0.84:  #在距离嘴巴比较远的位置时，如果conf>0.8则判断为烟头
...
if __name__ == '__main__':
    ...
    images_path = "inference\images\\50_50_test"        # 存放照片文件的位置
    ...
    for j in range(len(images_file)):
        ...
        parser.add_argument('--source', type=str, default='./inference/images/50_50_test/{}/'.format(images_file[j]), help='source')  # file/folder, 0 for webcam
        parser.add_argument('--output', type=str, default='inference/output/50_50_test/original_less_fps/061/{}/'.format(images_file[j]), help='output folder')  # output folder
        parser.add_argument('--excel_file', type=str, default='./inference/output/50_50_test/original_less_fps/061/{}.xlsx'.format(images_file[j]), help='excel folder')  # output folder
        ...
        with torch.no_grad():
            if opt.update:  # update all models (to fix SourceChangeWarning)
                ...
                for i in range(total_num):
                    ...
                    #source_file = './inference/images/smoke_true_img/{}'.format(all_files[i])
                    #output_file = 'inference/output/smoke_ture_img/{}'.format(all_files[i])
                    print(source_file,output_file)

                    start = time.time()
                    Smoke_flag,picture_num,picture_total = detect(output_file=output_file,source_file=source_file)
                    end = time.time()
