...
def get_graph(network_data, vocab):
    ...
    data_dict = dict()
    num_nodes_dict = {node_type: len(vocab)}  # 节点数量

    for edge_type in network_data:
        tmp_data = network_data[edge_type]  # 每个类别的节点
        src = []
        dst = []
        for edge in tmp_data:
            src.extend([vocab[edge[0]], vocab[edge[1]]])  # src -> dst
            dst.extend([vocab[edge[1]], vocab[edge[0]]])
        data_dict[(node_type, edge_type, node_type)] = (src, dst)
    graph = dgl.heterograph(data_dict, num_nodes_dict)  # 创建异构图
...
def train_model(network_data):
    ...
    num_nodes = len(index2word)  # node数量
    ...
    for i in range(edge_type_count):
        nodes = torch.LongTensor(type_nodes[i] * args.num_walks)  # 游走次数
        traces, types = dgl.sampling.random_walk(
            g, nodes, metapath=[edge_types[i]] * (neighbor_samples - 1)  # len(metapath) + 1
        )
...
if __name__ == "__main__":
    ...
    training_data_by_type = load_training_data(file_name + "/train.txt")  # 导入训练数据
