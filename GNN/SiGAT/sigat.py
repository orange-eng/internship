...
class AttentionAggregator(nn.Module):
    ...
    def forward(self, nodes, adj):
        ...
        unique_nodes_list = np.unique(np.hstack((np.unique(edges), np.array(nodes))))  # 求edge和nodes的节点集合; 所有涉及到的节点
...
def run( dataset='bitcoin_alpha', k=2):
    ...
    def func(adj_list):
        ...
        adj = sp.csr_matrix((np.ones(len(edges)), (edges[:,0], edges[:,1])), shape=(num_nodes, num_nodes))  # 构建稀疏矩阵
        return adj
