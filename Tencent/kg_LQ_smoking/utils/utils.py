...
def non_max_suppression(prediction, conf_thres=0.3, iou_thres=0.6, merge=False, classes=None, agnostic=False):
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    other_c1,other_c2 = (int(x[0]),int(x[3])),(int(x[2]),int(x[1]))

    cv2.circle(img,other_c1,5,(0,0,255),4)
    cv2.circle(img,other_c2,5,(0,0,255),4)
    cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        # cv2.circle(img,c1,5,(0,0,255),4)
        # cv2.circle(img,c2,5,(0,0,255),4)
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    for i, img in enumerate(images):
        ...
        cv2.circle(mosaic,(block_x,block_y),1,(0,0,255),4)
        cv2.circle(mosaic,(block_x + w, block_y + h),1,(0,0,255),4)
