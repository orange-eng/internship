    ...
    with open(f_name, "r") as f:
        for line in f:
            ...
            if words[0] not in edge_data_by_type:
                edge_data_by_type[words[0]] = list()  # label
            x, y = words[1], words[2]  # src, dst
            edge_data_by_type[words[0]].append((x, y))
            all_nodes.append(x)
            all_nodes.append(y)
    all_nodes = list(set(all_nodes))  # 节点去重
...
def load_testing_data(f_name):
    ...
    with open(f_name, "r") as f:
        for line in f:
            words = line[:-1].split(" ")
            x, y = words[1], words[2]
            if int(words[3]) == 1:  # true的dict; 边预测是否存在
    ...
    for walk in walks:
        ...
        for i in range(len(walk)):
            for j in range(1, skip_window + 1):
                if i - j >= 0:
                    pairs.append((walk[i], walk[i - j], layer_id))  # 向前推skip_window
                if i + j < len(walk):
                    pairs.append((walk[i], walk[i + j], layer_id))  # 向后推skip_window
    return pairs
...
def generate_pairs(all_walks, window_size, num_workers):
    ...
    return np.array([list(pair) for pair in set(pairs)])  # 转换成array格式


def generate_vocab(network_data):
    nodes, index2word = [], []
    for edge_type in network_data:  # 遍历每个类别
        node1, node2 = zip(*network_data[edge_type])  # node1, node2的list
        index2word = index2word + list(node1) + list(node2)

    index2word = list(set(index2word))  # 节点去重
    vocab = {}
    i = 0
    for word in index2word:
        vocab[word] = i  # 单词编号index
        i = i + 1

    for edge_type in network_data:
        node1, node2 = zip(*network_data[edge_type])
        tmp_nodes = list(set(list(node1) + list(node2)))  # type节点去重
        tmp_nodes = [vocab[word] for word in tmp_nodes]  # 节点编号
        nodes.append(tmp_nodes)  # type类别，所有节点对应到的索引
    # index2word:全部节点; vocab:全部节点对应索引; nodes:每个type对应到的index
